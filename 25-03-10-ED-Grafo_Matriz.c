...
void LimparGrafo(Grafo * grafo){
  ...
  for(int i = 0; i< grafo->numVertices;i++){
    for(int j =0; j<grafo->numVertices;i++){
      grafo->matriz[i][j] = FALSE;
    }
  }
...
int RetornarGrau(Grafo* grafo, int vertice){
  ...
  int grau = 0;
  for (int i=0;i<grafo->numVertices;i++){
    // Verifica se o vertice é diferente do vertice que está sendo verificado
...
Boolean VerificarPossuiVizinho(Grafo* grafo, int vertice){
  // Apenas reaproveitando a função RetornarGrau
  int grau = RetornarGrau(grafo, vertice);
